...
#Invokes update modules provided in the arguments with --upgrade argument
function performUpgrade()
{
    echo "Programs to update ↓";

    for module in "$@"; do
        echo "-> ${module:13:${#module} - 16}";
    done

    echo;

    if [ $# -gt 1 ]; then
        echo "Elevated privileges are neccesery to perform updates.";
    else
        echo "Elevated privileges are neccesery to perform an update.";
    fi

    if ! sudo -v; then
        echo "Failed to elevate privileges. Upgrades canceled";
    ...
    for arg in "$@"; do
        ...
        local argStatus=$?;
        if [ "$argStatus" -eq 0 ]; then
            succeeded[${#succeeded[@]}]=${arg:13:${#arg} - 16};
            echo "Successfully ran \"$arg\" with exit code $argStatus";
        else
            failed[${#failed[@]}]=${arg:13:${#arg} - 16};
            echo "Module \"$arg\" failed to upgrade and quit with code $argStatus";
        fi
...
# TEMP => TODO remove it ↓
if get_is_update || get_is_upgrade || get_is_integrity_check; then perform_exit 0; fi

if [[ $1 == "--perform-upgrade" ]]; then
    
    if ! performUpgrade "${@:2}"; then
        echo "[ERR] Failed to perform upgrades!";
        exit 1;
    fi
    
    exit 0;
elif [[ $1 == "--check-for-update" ]]; then
    if [ -d "$HOME/Documents/UpdateUtility" ]; then
        cd "$HOME/Documents/UpdateUtility" || exit 2;
    elif [[ $PWD != "$HOME/Documents/UpdateUtility" ]]; then
        echo "[INFO] No \"$HOME/Documents/UpdateUtility/\" directory found!";
        exit 3;
    fi
    ...
    for file in "${files[@]}"; do
        if grep -Eq "[-][-r][-w]x[r-][-w][-x][-]{3}" <<< "${file:0:10}" && grep -Eq "UpdateModule([.](.)+)?[.]sh" <<< "${file:10}"; then
            echo "[INFO] Triggering update checker in module ${file:10}"$'\n';
            ./"${file:10}" --check-for-update;

            status=$?;
            if [ $status -eq 0 ]; then
                echo "[INFO] \"${file:10}\" -> $status (update available)";
                upgradable[${#upgradable[@]}]=${file:10};
            elif [ $status -eq 1 ]; then
                echo "[INFO] \"${file:10}\" -> $status (up to date)";
            else
                echo "[WARN] There's a problem with module -> \"${file:10}\". Returned code \"$status\"";
            fi
    ...
    #echo "[DEBUG][ARRAY]" "${upgradable[@]}";

    if [ ${#upgradable[@]} -gt 0 ]; then
        #gnome-terminal -- ./UpdateUtility.sh --perform-upgrade "${upgradable[@]}" > /dev/null 2> /dev/null;
        konsole -e uputil --perform-upgrade "${upgradable[@]}" > /dev/null 2> /dev/null;
    else
        echo "[INFO] No updates are available";
    fi
else
    echo "UpdateUtility usage ↓";
    echo "uputil [arg] [moduleNames...]";
    echo "possible args:";
